...
const Comments = () => {
  ...
  const onChooseComment = (e, id) => {
    ...
    if (id === 'all') {
      if (chosenComments?.length === comments?.length)
        setChosenComments(() => []);
      else
        setChosenComments(() => [
    ...
    if (checked) {
      ...
      setChosenComments([...chosenComments, id]);
    } else {
      const filteredList = [...chosenComments].filter(
        (payment) => payment !== id
      );
        ...
        <div className="users-list-container">
                ...
                <tr key={i}>
                  <th>
                    <input
                      type="checkbox"
                      onChange={(e) =>
                        onChooseComment(e, payment?.number_payment)
                      }
                  ...
                  <td>
                    {payment.mont_balance || 0}
                    <span className="currence-highlight">с</span>
                  </td>
                  <td>
                    {payment.old_avail_balance || 0}
                    <span className="currence-highlight">с</span>
                  </td>
                  <td>
                    {payment.new_avail_balance || 0}
                    <span className="currence-highlight">с</span>
                  </td>
                  <td>
                    {payment.mont_avail_balance || 0}
                    <span className="currence-highlight">с</span>
                  </td>
          ...
          <div className="pagination-container">
            ...
            >
              ...
              >
                {!commentsPagesAmount ||
                paginationData.page < commentsPagesAmount
                  ? 'Показать ещё...'
                  : 'Больше данных нет'}
              </CustomButton>
            </div>
